...
def applyCoder(text, coder):
    ...
    for letter in text:
        ...
        if letter in coder:
            ...
            encoded.append(coder.get(letter))
...
def findBestShift(wordList, text):
    ...
    for shift in range(0,26):
        ...
        test = applyShift(text, shift)
    
        #split the resulting text into word        
        ...
        #if the number of real words is the highest,:
            if count > total_words:
                #record the number of words
                total_words = count
                #set best shift to current shift
                best_shift = shift
