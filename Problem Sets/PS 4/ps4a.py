...
def updateHand(hand, word):
    ...
    for letter in word:
            updated[letter] = updated.get(letter, 0) - 1
...
def isValidWord(word, hand, wordList):
    ...
    for letter in handCopy.keys(): # gets the letters of the remaining hand (once letters of the played word have been removed)
        for j in range (handCopy[letter]): # in case there are more than one of same letter
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    if word == ".":
        ...
        if remainingHand == len(word):
            print("Run out of letters. Total score: ") + str(totalScore + tooMuch) + (" points.")
